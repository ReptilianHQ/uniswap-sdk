...
export function decodeV4PoolInitialization(deployment: V4Deployment, log: {
  ...
  const decoded = (() => {
    try {
      return decodeEventLog({ abi: v4PoolManagerAbi, data: log.data, topics: log.topics, strict: true });
    } catch (cause) {
      throw new UniswapSdkError('INVALID_ARGUMENT', 'Initialize log cannot be decoded', { cause });
    }
  })();
